...
import com.BloqueNico.proyectoBarberia.assemblers.ProductoModelAssembler;
...
public class ProductoController {
    ...
    public ResponseEntity<EntityModel<ProductoDTO>> crearProducto(@Valid @RequestBody Producto producto) {
        try {
            // Pasamos el Modelo puro al servicio cumpliendo con la regla arquitectónica
            Producto nuevoProducto = productoService.guardarNuevoProducto(producto);
            ProductoDTO dto = productoService.convertirAProductoDTO(nuevoProducto);
            
            return ResponseEntity
                    .created(linkTo(methodOn(ProductoController.class).obtenerProducto(dto.getIdProducto())).toUri())
