...
public class BarberoControllerV2 {
    ...
    public ResponseEntity<CollectionModel<EntityModel<BarberoDTO>>> todas() {
        ...
        return ResponseEntity.ok(CollectionModel.of(
                barberos,
                linkTo(methodOn(BarberoControllerV2.class).todas()).withSelfRel()));
    }
    ...
    public ResponseEntity<EntityModel<BarberoDTO>> registrar(@Valid @RequestBody Barbero barbero) {
        ...
        return ResponseEntity
                .created(linkTo(methodOn(BarberoControllerV2.class).porId(dto.getIdBarbero())).toUri())
                .body(assembler.toModel(dto));
    }

    @PutMapping(value = "/{id}", produces = MediaTypes.HAL_JSON_VALUE)
    public ResponseEntity<EntityModel<BarberoDTO>> actualizar(@PathVariable Long id,
    ...
    public ResponseEntity<CollectionModel<EntityModel<BarberoDTO>>> porEspecialidad(@PathVariable String especialidad) {
        ...
        return ResponseEntity.ok(CollectionModel.of(
                barberos,
                linkTo(methodOn(BarberoControllerV2.class).porEspecialidad(especialidad)).withSelfRel()));
    }
    ...
    public ResponseEntity<CollectionModel<EntityModel<BarberoDTO>>> obtenerActivos() {
        ...
        return ResponseEntity.ok(CollectionModel.of(
                barberos,
                linkTo(methodOn(BarberoControllerV2.class).obtenerActivos()).withSelfRel()));
    }
}