...
import com.BloqueNico.assemblers.ClienteModelAssembler;
...
public class ClienteControllerV2 {
    ...
    public ResponseEntity<EntityModel<ClienteDTO>> crear(@Valid @RequestBody ClienteDTO clienteDTO) {
        log.info("POST /api/v2/clientes - creando cliente con HATEOAS");
        ClienteDTO clienteCreado = servicioDeClientes.guardarNuevoCliente(clienteDTO);
        return ResponseEntity
                .created(linkTo(methodOn(ClienteControllerV2.class).traerUno(clienteCreado.getIdCliente())).toUri())
    ...
    public ResponseEntity<EntityModel<ClienteDTO>> actualizar(
        ...
        ClienteDTO clienteActualizado = servicioDeClientes.actualizarCliente(id, clienteDTO);
